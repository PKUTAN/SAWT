...
def cal_obj(sol,D,F):
    return np.sum(F*(sol@D@sol.T))

def cal_obj_init(D,F):
    sol = np.eye(F.shape[0])
    return np.sum(F*(sol@D@sol.T))

def normalize_adj(adj):
    # row_sum = torch.sum(adj,axis=-1,keepdims=True)
    # d_inv_sqrt = torch.pow(row_sum,-0.5)
    # d_inv_sqrt[torch.isinf(d_inv_sqrt)] = 0.
    # d_inv_sqrt = d_inv_sqrt.squeeze()
    
    # output = []
    # for i in range(adj.shape[0]):
    #     d_inv_sqrt_mat = torch.diag(d_inv_sqrt[i])
    #     output.append(d_inv_sqrt_mat @ adj[i] @ d_inv_sqrt_mat)

    # return torch.stack(output,dim=0)

    S = torch.sum(adj, dim=2)
    S_d = torch.diag_embed(1/torch.sqrt(S), offset=0, dim1=-2, dim2=-1)
    dist = torch.bmm(torch.bmm(S_d, adj), S_d)
    return dist

def pre_process_adj(adj):
    b = adj.shape[0]
    adj = normalize_adj(adj + torch.eye(adj.shape[1]).unsqueeze(0).repeat(b,1,1).to(device))
    return adj
...
def swap_2opt(tour, i, k):
    ...
    # new_tour = np.append(new_tour,tour[k])
    # new_tour = np.append(new_tour,tour[i+1:k])
    # new_tour = np.append(new_tour,tour[i])
...
def swap_new(tour,idx1,idx2,D,F):
    tour[idx1], tour[idx2] = tour[idx2], tour[idx1]
    new_tour = [x for x in tour]
    x_indice = [x for x in range(len(tour))]

    per = np.zeros((len(tour), len(tour)))

    per[x_indice, new_tour] = 1
    # import pdb; pdb.set_trace()
    return new_tour, cal_obj(per,D,F)
...
def batch_pair_squared_dist_no_norm(x, y):
    '''
    Modified from https://discuss.pytorch.org/t/efficient-distance-matrix-computation/9065/3
    Input: x is a bxNxd matrix y is an optional bxMxd matirx
    Output: dist is a bxNxM matrix where dist[b,i,j] is the square norm between x[b,i,:] and y[b,j,:]
